...
const sourceFiles =
  [
    ...
    `src/littlejs/engineMath.js`,
    `src/littlejs/engineUtilities.js`,
    `src/littlejs/engineSettings.js`,
    `src/littlejs/engineObject.js`,
    `src/littlejs/engineDraw.js`,
    `src/littlejs/engineInput.js`,
    `src/littlejs/engineAudio.js`,
    `src/littlejs/engineTileLayer.js`,
    `src/littlejs/engineParticles.js`,
    `src/littlejs/engineMedals.js`,
    `src/littlejs/engineWebGL.js`,
    `src/littlejs/engine.js`,

    // game files
    'dist/game.js',
  ];

const dataFiles = ['public/t.gif'];
...
try {
  ...
  const dataFilesForZip = [];
  for (const file of dataFiles) {
    const filename = path.basename(file); // Extracts 't.gif' from 'public/t.gif'
    fs.copyFileSync(file, `${BUILD_FOLDER}/${filename}`);
    dataFilesForZip.push(filename);
  }
...
function closureCompilerStep(filename) {
  ...
  try {
    ...
    // execSync(`npx google-closure-compiler --js=${filenameTemp} --js_output_file=${filename} --compilation_level=ADVANCED --warning_level=VERBOSE --jscomp_off=* --assume_function_wrapper --preserve_type_annotations`, { stdio: 'inherit' });
  }
...
function zipBuildStep(filename) {
  ...
  // Extract just the filenames (e.g., 't.gif') instead of 'public/t.gif'
  const flatDataFiles = dataFiles.map(file => path.basename(file));
  const args = ['-9', '-strip', '-zip', `../${PROGRAM_NAME}.zip`, 'index.html', ...flatDataFiles];
